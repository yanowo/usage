...
`usage` 是一個顯示 **Claude Code 跟 Codex** 用量的小工具。macOS 可以跑在 menu bar（螢幕右上角的選單列），Windows 會跑成 Web 介面，讓瀏覽器或桌面擺件直接用 URL 查看目前 5 小時、7 天、今日 token 用量與成本估算。
...
| Windows 使用者，想用桌面擺件 URL | [Web 模式](#web-模式windows-預設) |
| 開發者，想從原始碼跑 | [建環境](#建環境) |
...
Windows 沒有 macOS menu bar，所以預設會啟動一個本機 Web server。啟動後把網址貼到瀏覽器或桌面擺件（例如可顯示網頁 URL 的 widget）就能查看。
...
- `http://127.0.0.1:8765/`：完整 Web 面板
- `http://127.0.0.1:8765/compact`：適合桌面擺件的小尺寸面板
- `http://127.0.0.1:8765/api/usage`：JSON API

如果你的桌面擺件只需要一個 URL，請填 `http://127.0.0.1:8765/compact`。要改 port 可用：
...
- **Web / Windows 使用者** —— 從原始碼啟動 `python main.py --web`，用瀏覽器或桌面擺件打開 URL。
...
| Windows 打開 `python main.py` 沒有跳視窗 | Windows 預設是本機 Web server，不會開原生視窗 | 看終端機印出的網址，或直接打開 `http://127.0.0.1:8765/compact` |
